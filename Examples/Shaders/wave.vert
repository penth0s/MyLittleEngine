...
// ----------------------------------------------------------
// Perlin Classic 3D - Original Implementation
// ----------------------------------------------------------
vec4 permute(vec4 x){
...
// ----------------------------------------------------------
// Waves Function - Original waves.glsl equivalent
// ----------------------------------------------------------
vec2 waves(vec3 position, float wavesStrengh, vec2 wavesFreq)
{
    ...
    float totalSmallWaves = 0.0;
    for(float i = 1.0; i <= 2.0; i++)
    {
        float smallWave = abs(perlinClassic3D(vec3(position.xz * 3.0 * i, uTime * 0.2)) 
                              * wavesStrengh * 0.5 / i);
...
void main()
{
    ...
    vec4 modelPosition = uModel * vec4(aPosition, 1.0);
    vPosition = modelPosition;
    
    if(modelPosition.y > -0.5)
    {
        vec2 yWaves = waves(modelPosition.xyz, uWavesStrength, uWavesFreq);
        modelPosition.y += yWaves.x;
        modelPosition.y = max(modelPosition.y, -1.0f);
        vElevation = yWaves;
    }
    else
    {
    ...
    vec4 projectedPosition = uProj * viewPosition;
